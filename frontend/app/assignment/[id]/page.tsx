...
import { printPDF } from "../../lib/printPdf";
import { useUserStore } from "../../store/userStore";
import { Printer, PlusCircle, XCircle, Home, RefreshCw, Loader2, CreditCard, LayoutDashboard, Eye, EyeOff, Settings } from "lucide-react";
import toast from "react-hot-toast";
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 py-4 sm:py-6 lg:py-8 pb-20 sm:pb-8">
          ...
          <div className="hidden sm:flex justify-between items-center mb-6 no-print">
            ...
            <div className="flex gap-3">
              ...
              {/* Settings Button with Difficulty Toggle */}
            ...
            {fixedSections.map((section, sectionIdx) => (
              <div key={sectionIdx} className="mb-4">
                ...
                {section.questions?.map((q, qIdx) => {
                  ...
                  // Format: (X Marks) + Difficulty if enabled
                  ...
                  // Get difficulty color for badge (only for screen, not print)
                  ...
                  return (
                    <div key={qIdx} className="mb-3 question-item">
                      ...
                      {/* Difficulty badge for screen view only */}
                      {showDifficulty && q.difficulty && (
                        <div className="no-print inline-block ml-6 mt-1">
                          <span className={`inline-block text-xs px-2 py-0.5 rounded-full border ${getDifficultyColor(q.difficulty)}`}>
                            {q.difficulty}
                          </span>
                        </div>
                      )}
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-amber-100 py-3 px-4 sm:hidden no-print shadow-lg z-10">
            ...
            <div className="flex justify-around items-center">
              ...
              {/* Mobile Settings Button with Difficulty Toggle */}
              <div className="relative">
                <button
                  onClick={() => setShowSettings(!showSettings)}
                  className="flex flex-col items-center gap-1 text-gray-600 hover:text-amber-600 transition-colors"
