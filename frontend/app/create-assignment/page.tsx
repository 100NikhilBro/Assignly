  ...
  const handleSubmit = async () => {
    ...
    if (currentCredits <= 0) {
      const errorMsg = isAuthenticated 
        ? "No credits left. Please upgrade to continue."
        : "Guest credits exhausted. Please login to continue.";
      setError(errorMsg);
    ...
    try {
      ...
      // Prepare session ID for guest users
  ...
  headers["Authorization"] = `Bearer ${token}`;
      ...
      if (response.data.creditsRemaining !== undefined) {
        if (user) {
          ...
          // Update guest credits state
          setGuestCredits(response.data.creditsRemaining);
        }
      }
