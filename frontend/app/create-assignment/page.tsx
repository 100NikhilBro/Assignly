  ...
  return (
    <div className="relative min-h-screen bg-[#fdfaf5]">
      ...
      <div className="relative max-w-4xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-amber-50/80 backdrop-blur-sm px-4 py-2 rounded-full mb-4 border border-amber-100">
            <Sparkles className="w-4 h-4 text-amber-600" />
            <span className="text-sm text-amber-700 font-medium">Create New Assignment</span>
          </div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Create Assignment
          </h1>
        ...
        {/* Error Alert */}
        {error && (
          <div className="mb-6 bg-red-50/90 backdrop-blur-sm border border-red-200 text-red-600 p-4 rounded-xl flex items-center gap-2">
            <AlertCircle className="w-5 h-5 flex-shrink-0" />
            <span>{error}</span>
          </div>
        )}

        <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-md border border-amber-100 p-6 md:p-8">
          <div className="space-y-6">
            {/* Basic Info Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  value={form.schoolName}
                  onChange={handleChange}
                  className="w-full bg-[#fdfaf5] border border-amber-200 rounded-xl pl-10 pr-4 py-3 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition"
                />
              </div>
              <div className="relative">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={form.dueDate}
                  onChange={handleChange}
                  className="w-full bg-[#fdfaf5] border border-amber-200 rounded-xl pl-10 pr-4 py-3 text-gray-900 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyPress={(e) => e.key === "Enter" && addConcept()}
                  placeholder="Add concepts (e.g., Photosynthesis, Algebra)"
                  className="flex-1 bg-[#fdfaf5] border border-amber-200 rounded-xl px-4 py-3 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                />
                <button
                  onClick={addConcept}
                  className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl transition flex items-center gap-2"
                >
                  <Plus className="w-4 h-4" />
                  Add
                </button>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {form.concepts.map((concept) => (
                  <span
                    key={concept}
                    className="bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm flex items-center gap-2 border border-amber-200"
                  >
                    {concept}
                    <button
                      onClick={() => removeConcept(concept)}
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                rows={3}
                className="w-full bg-[#fdfaf5] border border-amber-200 rounded-xl px-4 py-3 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
              />
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {DIFFICULTY_OPTIONS.map((opt) => (
                  ...
                  >
                    <opt.icon className={`w-5 h-5 mb-2 ${form.difficultyLevel === opt.value ? "text-amber-600" : "text-amber-500"}`} />
                    <div className="font-medium text-gray-900">{opt.label}</div>
                    <div className="text-xs text-gray-500 mt-1">{opt.description}</div>
                  </button>
                ))}
            ...
            {/* Credit Info */}
            <div className="bg-gradient-to-r from-amber-50/80 to-amber-100/50 p-4 rounded-xl border border-amber-200 backdrop-blur-sm">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <Star className="w-5 h-5 text-amber-600" />
                  <span className="text-gray-700">Credits remaining:</span>
                  <span className="text-2xl font-bold text-amber-700">{getCurrentCredits()}</span>
                </div>
                {!user && (
                  <button
                    onClick={() => router.push("/login")}
            ...
            <button
              onClick={handleSubmit}
              disabled={loading || getCurrentCredits() <= 0}
              className="w-full bg-amber-600 hover:bg-amber-700 text-white py-4 rounded-xl font-semibold text-lg transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
                  <Loader2 className="w-5 h-5 animate-spin" />
